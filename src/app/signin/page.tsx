...
import { testDatabaseConnection } from '@/lib/firebase/firebaseUtils';
...
export default function SignIn() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      ...
      setDbStatus(isConnected ? 'Connected to Firebase' : 'Failed to connect to Firebase');
      console.log('Database status:', isConnected ? 'Connected' : 'Failed to connect');
    };
    checkConnection();
  }, []);
        ...
        <div>
          ...
          {dbStatus && (
            <p className={`mt-2 text-center text-sm ${
              dbStatus.includes('Connected') ? 'text-green-600' : 'text-red-600'
            }`}>
              {dbStatus}
            </p>
          )}
